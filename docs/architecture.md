...
| 技術 | バージョン | 備考 |
|------|-----------|------|
| Node.js | 24.x LTS | Next.js 16が推奨するLTSバージョン |
| TypeScript | 6.x | 既存プロジェクトのスタック |
...
| 技術 | バージョン | 用途 | 選定理由 |
|------|-----------|------|----------|
| Next.js | 16.3.2 | Webフレームワーク | 既存プロジェクトのスタック。App RouterによるServer/Client Component分離が可能 |
| React | 19.2.8 | UIライブラリ | Next.js 16と組み合わせ。Suspenseによるローディング制御に対応 |
| Prisma | 7.9.1 | ORM | 既存プロジェクトのスタック。TypeSafe なDB操作、マイグレーション管理 |
| Tailwind CSS | 4.3.3 | スタイリング | 既存プロジェクトのスタック。設定は globals.css の @theme に置く（CSS-first） |
...
| ライブラリ | 用途 | バージョン管理方針 |
|-----------|------|-------------------|
| @prisma/client | DBアクセス | 既存固定（7.9.1）、スキーマ変更時に合わせて更新 |
| next | フレームワーク | 既存固定（16.3.2）|
| react | UIライブラリ | 既存固定（19.2.8）|
| tailwindcss | スタイリング | 既存固定（4.3.3）|
